...
app.layout = html.Div(children=[
    ...
    dcc.Dropdown(
        ...
        options=[{'label': category, 'value': category} for category in
                 df_ratp_trafic.groupby(['Réseau']).groups.keys()],
        value=None,
    ...
    html.Div(className="ratp_Charts", style={'display': 'Flex'}, children=[
        dcc.Graph(
            ...
            figure=px.bar(df_ratp_trafic, x=df_ratp_trafic["Station"], y=df_ratp_trafic["Trafic"],
                          title="Traffic par Station")
        ),
        dcc.Graph(
            className='pie_chart_ville',
            id='pie_chart_ville',
            figure=px.pie(df_ratp_ville, df_ratp_ville.index, df_ratp_ville["Trafic"],
                          title="Traffic par station (5 plus grosses stations uniquement)").update_layout(
                showlegend=False, )
    ...
    html.Div(className="separeteur"),
    html.P(
        "Choissiez un exploitant pour filtrer les résultats exposer dans le graphique de nombre de station par ligneà"),
    dcc.Dropdown(
    ...
    html.Div(className="map", children=[
        html.P(
            "vous pouvez cliquer sur les noms des lignes dans la légende pour effacer ou faire apparaitre les points."),
        dcc.Graph(
            id="map",
            figure=fig
        ),
